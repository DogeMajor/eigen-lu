...
template<typename Scalar, int StorageOrder, typename PivIndex, int RowSize, int ColSize>
struct partial_lu_impl
{
  ...
  {
	//cout << "Not using template specialization for size==(m,n)..." << endl;
...
struct partial_lu_impl<Scalar, StorageOrder, PivIndex, 2, 2>
{
	...
	inline static Index unblocked_lu(MatrixType& lu, PivIndex* row_transpositions, PivIndex& nb_transpositions)
	{
		...
		Index row_of_biggest_in_col;
		Score biggest_in_corner
			= lu.col(0).tail(2).unaryExpr(Scoring()).maxCoeff(&row_of_biggest_in_col);
...
struct partial_lu_impl<Scalar, StorageOrder, PivIndex, 3, 3>
{
	...
	inline static Index unblocked_lu(MatrixType& lu, PivIndex* row_transpositions, PivIndex& nb_transpositions)
	{
		cout << "Using template specialization for size==(3, 3)..." << endl;
		typedef scalar_score_coeff_op<Scalar> Scoring;
		typedef typename Scoring::result_type Score;
		const Index rows = 3;
		const Index cols = 3;
...
struct partial_lu_impl*/
  //RowSize = MatrixType::RowsAtCompileTime
  //ColSize = MatrixType::ColsAtCompileTime
  partial_lu_impl
